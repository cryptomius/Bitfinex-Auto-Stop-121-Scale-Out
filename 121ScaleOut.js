...
let {
  p: tradingPair, a: tradeAmount, e: entryPrice, s: stopPrice, S: slippage,
...
rest.ticker('t' + tradingPair).then((ticker) => {
  const tickerObj = ticker.toJS()
  let currentPrice = tickerObj.lastPrice

  let isShort = entryPrice ? entryPrice < stopPrice : currentPrice < stopPrice
  let entryLimitOrder = entryPrice &&
    ((!isShort && currentPrice > entryPrice) || (isShort && currentPrice < entryPrice))
